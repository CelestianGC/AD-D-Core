...
function onHover(tokenMap, bOver)
	...
	if nodeCT then
        ...
        if User.isHost() then
          sOption = OptionsManager.getOption("DM_SHOW_NPC_HEALTHBAR");
        else 
        ...
        if User.isHost() then
          OptionsManager.getOption("DM_SHOW_NPC_EFFECTS");
        else
...
function updateNameHelper(tokenCT, nodeCT)
	...
	if sOptTNAM ~= "on" and (sOptTNAM == "off" or sOptTNAM == "tooltip") then
		for _, vWidget in pairs(aWidgets) do
			vWidget.destroy();
		end
	else
