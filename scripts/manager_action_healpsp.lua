...
function onInit()
  ActionsManager.registerModHandler("heal_psp", modHeal);
  ActionsManager.registerResultHandler("heal_psp", onHeal);
end

function getRoll(rActor, rAction)
  local rRoll = {};
  rRoll.sType = "heal_psp";
  rRoll.aDice = {};
  rRoll.nMod = 0;
...
function modHeal(rSource, rTarget, rRoll)
  ...
  if rSource then
    ...
    local nEffectCount;
    aAddDice, nAddMod, nEffectCount = EffectManager5E.getEffectsBonus(rSource, {"HEALPSP"});
